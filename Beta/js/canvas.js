...
sphere.prototype = {
    update: function () {
        ...
        for (var i = 0; i < this.charsMap[this.type].length; i++) {
            ...
            this.degree[i].theta =
                this.degree[i].theta + (this.charsMap[this.type][i].theta - this.degree[i].theta) / (4 + 20 * Math.random());
...
document.body.onmousemove = function (e) {
    camera.rotate.x = (e.pageY / window.innerHeight) * canvasParallax - canvasParallax / 2;
    camera.rotate.y = (e.pageX / window.innerWidth) * canvasParallax - canvasParallax / 2;
...
setTimeout(() => {
    if (canScroll) document.getElementById("content").scrollIntoView();
}, canvasTime * textSet.length + 1000);
